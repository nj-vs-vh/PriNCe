...
def get_decay_matrix(mo, da, x_grid):
    ...
    # neutron
        ...
        info(5, f"Called with unknown channel {mo} to {da}, returning an empty redistribution")
...
def nu_from_beta_decay(x_grid, mother, daughter, Gamma=200, angle=None):
    ...
    if mother == 100 and daughter == 101:
        ...
        raise Exception(f"Not an allowed beta decay channel: {mother} -> {daughter}")
    ...
    with warnings.catch_warnings():
        ...
        res = E_mesh**2 / boost_mesh**5 * (Emax - E_mesh) * np.sqrt((E_mesh - Emax) ** 2 - boost_mesh**2 * mass_el**2)
    res[E_mesh > Emax] = 0.0
    res = np.nan_to_num(res)

    if np.all(res == 0):
        info(10, "Differential distribution is all zeros for", mother, daughter, "No angle averaging performed!")
    elif angle is None:
...
def nu_from_beta_decay_old(x_grid, mother, daughter):
    ...
    norm = (
        1.0
        / 60.0
        * (np.sqrt(1.0 - ye**2) * (2 - 9 * ye**2 - 8 * ye**4) + 15 * ye**4 * np.log(ye / (1 - np.sqrt(1 - ye**2))))
    )
    ...
    # total formula
    result[cond] = subst / norm * yshort**2 * (1 - yshort) * np.sqrt((1 - yshort) ** 2 - ye**2)

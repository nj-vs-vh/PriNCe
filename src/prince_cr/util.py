...
def get_interp_object(xgrid, ygrid, **kwargs):
    ...
    if xgrid.shape != ygrid.shape:
        raise Exception(f"xgrid and ygrid args need identical shapes: {xgrid.shape} != {ygrid.shape}")
...
class AdditiveDictionary(dict):
    ...
    def __setitem__(self, key, value):
        if key not in self:
            ...
            super(AdditiveDictionary, self).__setitem__(key, (self[key][0], value[1] + self[key][1]))
        else:
...
class PrinceBDF(BDF):
    ...
    def _step_impl(self):
        ...
        while not step_accepted:
            ...
            safety = round(0.9 * (2 * NEWTON_MAXITER + 1) / (2 * NEWTON_MAXITER + n_iter), ndigits=15)

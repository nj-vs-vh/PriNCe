...
class TheInterpolator:
    ...
    def _init_grids(self):
        ...
        for i in range(1, self.n_ext + nwi2 + 1):
            self.bins_ext[nwi2 + self.n_ext - i] = self.bins[0] * 10 ** (-grid_spacing * i)
            self.bins_ext[nwi2 + self.n_ext + self.bins.size + i - 1] = self.bins[-1] * 10 ** (grid_spacing * i)
    ...
    def set_initial_spectrum2(self, fx, fy):
        self.b *= 0.0
        for m in range(0, self.n_window):
            self.b[self.n_ext * self.n_window + m : -self.n_ext * self.n_window + m : self.n_window] += fy * fx**m
    ...
    def get_moment(self, m):
        ...
        return np.sum(
            self.widths_ext[nwi2:-nwi2] * self.grid_ext[nwi2:-nwi2] ** m * self.sum_mat.dot(self.solver(self.b))
        )

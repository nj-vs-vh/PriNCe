...
class UHECRPropagationResult:
    ...
    def __mul__(self, number):
        if not np.isscalar(number):
            raise Exception(f"Can only multiply result by scalar number, got type {type(number)} instead!")
        else:
    ...
    def _collect_interpolated_spectra(self, nco_ids, epow, egrid=None):
        ...
        for idx, pid in enumerate(nco_ids):
            ...
            if np.count_nonzero(mask) > 0:
                res = np.exp(
                    np.interp(
                        np.log(com_egrid), np.log(curr_egrid[mask]), np.log(curr_spec[mask]), left=np.nan, right=np.nan
                    )
                )
            else:
    ...
    def get_lnA(self, nco_ids, egrid=None):
        ...
        variance = (lnA[:, np.newaxis] ** 2 * spectra).sum(axis=0) / spectra.sum(axis=0) - average**2
...
class UHECRPropagationSolver:
    ...
    def semi_lagrangian(self, delta_z, z, state):
        ...
        # -------------------------------------------------------------------
            ...
            state[:] = state + self.dldz(z) * delta_z * self.diff_operator.dot(conloss * state)

        # -------------------------------------------------------------------
        # if method was not in list before, raise an Expection
        # -------------------------------------------------------------------
        else:
            raise Exception(f"Unknown semi-lagrangian method ({method})")
    ...
    def eqn_deriv_cupy(self, z, state, *args):
        ...
        if self.enable_partial_diff_jacobian:
            ...
            if self.enable_pairprod_losses:
                conloss += self.pair_loss_rates_grid.loss_vector(z)
            r += self.dldz(z) * self.diff_operator.dot(cupy.array(conloss)[:, np.newaxis] * state)
    ...
    def eqn_deriv_mkl(self, z, state, *args):
        ...
        if self.enable_partial_diff_jacobian:
            ...
            if self.enable_pairprod_losses:
                conloss += self.pair_loss_rates_grid.loss_vector(z)
            if matrix_input:
                res = csrmm(self.dldz(z), self.diff_operator, conloss[:, np.newaxis] * state, 1.0, res)
            else:
                res = csrmv(self.dldz(z), self.diff_operator, conloss[:, np.newaxis] * state, 1.0, res)
...
class UHECRPropagationSolverBDF(UHECRPropagationSolver):
    ...
    def solve(self, dz=1e-3, verbose=True, summary=False, extended_output=False, full_reset=False, progressbar=False):
        ...
        info(2, f"Solver initialized in {time() - start_time} s")

        info(2, "Starting integration.")
        with PrinceProgressBar(bar_type=progressbar, nsteps=-(self.initial_z - self.final_z) / dz) as pbar:
            while self.r.status == "running":
                # print '------ at', self.r.t, '------'
                if verbose:
                    info(3, f"Integrating at z = {self.r.t}")
                # --------------------------------------------
                # Evaluate a step
                # --------------------------------------------
                self.r.step()
        ...
        if self.r.status == "failed":
            raise Exception(f"Integrator failed at t = {self.r.t}, try adjusting the tolerances")
        if verbose:
            print(f"Integrator finished with t = {self.r.t}, last step was dt = {self.r.step_size}")
        ...
        end_time = time()
        info(2, f"Integration completed in {end_time - start_time} s")
...
class UHECRPropagationSolverEULER(UHECRPropagationSolver):
    ...
    def solve(
        self, dz=1e-3, verbose=True, extended_output=False, initial_inj=False, disable_inj=False, progressbar=False
    ):
        from time import time
        ...
        with PrinceProgressBar(bar_type=progressbar, nsteps=-(self.initial_z - self.final_z) / dz) as pbar:
            while curr_z + dz >= self.final_z:
                if verbose:
                    info(3, f"Integrating at z={curr_z}")

...
class PriNCeRun:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config.grid_scale == "E":
            ...
            raise Exception(f"Unknown energy grid scale {config.grid_scale}, adjust config.grid_scale")
        ...
        if "photon_field" in kwargs:
            ...
            self.photon_field = pf.CombinedPhotonField([pf.CMBPhotonSpectrum, pf.CIBGilmore2D])
        ...
        if "species_list" in kwargs:
            ...
            system_species = [s for s in self.cross_sections.known_species if get_AZN(s)[0] <= max_mass]
        ...
        self.adia_loss_rates_grid = interaction_rates.ContinuousAdiabaticLossRate(prince_run=self, energy="grid")
        self.pair_loss_rates_grid = interaction_rates.ContinuousPairProductionLossRate(prince_run=self, energy="grid")
        self.adia_loss_rates_bins = interaction_rates.ContinuousAdiabaticLossRate(prince_run=self, energy="bins")
        self.pair_loss_rates_bins = interaction_rates.ContinuousPairProductionLossRate(prince_run=self, energy="bins")

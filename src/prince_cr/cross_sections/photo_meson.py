...
class SophiaSuperposition(CrossSectionBase):
    ...
    def incl(self, mother, daughter):
        ...
        if daughter in [mother - 101]:
            ...
            raise Exception(f"Channel {mother} to {daughter} not allowed in this superposition model")
    ...
    def incl_diff(self, mother, daughter):
        ...
        if daughter > 101:
            raise Exception("Redistribution function requested for boost conserving particle")
        csec_diff = None
...
class EmpiricalModel(SophiaSuperposition):
    ...
    def incl_diff(self, mother, daughter):
        ...
        if (mother, daughter) in self.multiplicity:
            ...
            cs_incl_pi0_sophia = superposition_incl(mother, daughter) * float(Am) ** (-1 / 3.0)
            ...
            alpha_plus = np.where(egrid <= 1.0, 2.0 / 3, 1 - np.exp(-4.0 / 7 * (egrid - 1) ** 0.5) / 3)

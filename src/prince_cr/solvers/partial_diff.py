...
class DifferentialOperator:
    ...
    def construct_differential_operator(self):
        ...
        op_matrix[0, np.asarray(diags_leftmost)] = np.asarray(coeffs_leftmost) / (denom_leftmost * h)
        op_matrix[1, 1 + np.asarray(diags_left_1)] = np.asarray(coeffs_left_1) / (denom_left_1 * h)
        op_matrix[2, 2 + np.asarray(diags_left_2)] = np.asarray(coeffs_left_2) / (denom_left_2 * h)
        op_matrix[last, last + np.asarray(diags_rightmost)] = np.asarray(coeffs_rightmost) / (denom_rightmost * h)
        op_matrix[last - 1, last - 1 + np.asarray(diags_right_1)] = np.asarray(coeffs_right_1) / (denom_right_1 * h)
        op_matrix[last - 2, last - 2 + np.asarray(diags_right_2)] = np.asarray(coeffs_right_2) / (denom_right_2 * h)
    ...
    def solve_coefficients(self, stencils, degree=1):
        ...
        if len(stencils) < degree + 1:
            raise Exception(
                f"Not enough stencils to solve for dervative of degree {degree}, stencils given: {stencils}"
            )

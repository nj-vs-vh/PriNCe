...
from prince_cr.util import bin_widths, get_AZN, info


class CrossSectionBase(metaclass=ABCMeta):
    ...
    def set_range(self, e_min=None, e_max=None):
        ...
        info(5, f"Setting range to {e_min:3.2e} - {e_max:3.2e}")
        self._range = np.where((self._egrid_tab >= e_min) & (self._egrid_tab <= e_max))[0]
        info(
            2,
            f"Range set to {np.min(self._egrid_tab[self._range]):3.2e} - {np.max(self._egrid_tab[self._range]):3.2e}",
        )
    ...
    def is_differential(self, mother, daughter):
        ...
        #     return False
        if daughter <= config.redist_threshold_ID or (mother, daughter) in self.incl_diff_idcs:
    ...
    def _optimize_and_generate_index(self):
        ...
        for mo, da in self.incl_idcs:
            if da >= 100 and get_AZN(da)[0] > get_AZN(mo)[0]:
                raise Exception(f"Daughter {da} heavier than mother {mo}. Physics??")
            ...
            if self.is_differential(mo, da):
                ...
                # to _incl_diff_tab
                self._incl_diff_tab[(mo, da)] = self._arange_on_xgrid(self._incl_tab.pop((mo, da)))
    ...
    def _reduce_channels(self):
        ...
        info(
            3,
            (
                "Before optimization, the number of known primaries is {0} with " + "in total {1} inclusive channels"
            ).format(len(self._nonel_tab), len(self._incl_tab)),
        )

        if self.xbins is None:
            info(4, "Model does not provide a native xbins. Assuming JH special sophia", "binning.")
        ...
        def decay_cached(mother, daughter):
            dec_dist = int_scale * decs.get_decay_matrix_bin_average(mother, daughter, dec_bins_lower, dec_bins_upper)

            return dec_dist

        def convolve_with_decay_distribution(diff_dist, mother, daughter, branching_ratio):
        ...
        def follow_chain(first_mo, da, csection, reclev):
            ...
            if spec_data[da]["lifetime"] >= threshold:
                if self.is_differential(None, da):
                    ...
                    info(
                        20,
                        dbg_indent(reclev),
                        f"daughter {da} stable and differential. Adding to ({first_mo}, {da})",
                    )
                    new_dec_diff_tab[(first_mo, da)] = csection
                else:
                    info(20, dbg_indent(reclev), f"daughter {da} stable. Adding to ({first_mo}, {da})")
            ...
            for br, daughters in spec_data[da]["branchings"]:
                info(
                    10,
                    dbg_indent(reclev),
                    ("{3} -> {0:4d} -> {2:4.2f}: {1}").format(da, ", ".join(map(str, daughters)), br, first_mo),
                )
                ...
                for chained_daughter in daughters:
                    ...
                    if self.is_differential(None, chained_daughter):
                        ...
                        follow_chain(
                            first_mo,
                            chained_daughter,
                            convolve_with_decay_distribution(self._arange_on_xgrid(csection), da, chained_daughter, br),
                            reclev + 1,
                        )
                    else:
                        follow_chain(first_mo, chained_daughter, br * csection, reclev + 1)
        ...
        for mother in sorted(self._nonel_tab.keys()):
            if mother not in spec_data or spec_data[mother]["lifetime"] < threshold:
                info(20, f"Primary species {mother} does not fulfill stability criteria.")
        ...
        for mother, daughter in self.incl_idcs:
            if mother not in self.nonel_idcs:
                info(30, f"Removing {mother}/{daughter} from incl, since mother not stable ")
                _ = self._incl_tab.pop((mother, daughter))

            elif self.is_differential(mother, daughter):
                # Move the distributions which are expected to be differential
                # to _incl_diff_tab
                self._incl_diff_tab[(mother, daughter)] = self._arange_on_xgrid(self._incl_tab.pop((mother, daughter)))

        self._update_indices()

        for mother, daughter in self.incl_diff_idcs:
            if mother not in self.nonel_idcs:
                info(30, f"Removing {mother}/{daughter} from diff incl, since mother not stable ")
        ...
        info(
            3,
            (
                "After optimization, the number of known primaries is {0} with " + "in total {1} inclusive channels"
            ).format(len(self._nonel_tab), len(self._incl_tab) + len(self._incl_diff_tab)),
        )
    ...
    def nonel(self, mother):
        ...
        if mother not in self._nonel_tab:
            raise Exception(f"Mother {mother} unknown.")
    ...
    def incl(self, mother, daughter):
        ...
        if (mother, daughter) in self._incl_diff_tab:
            ...
            cs_incl = trapezoid(cs_diff, x=self.xcenters, dx=bin_widths(self.xbins), axis=0)
    ...
    def incl_diff(self, mother, daughter):
        ...
        if (mother, daughter) not in self._incl_diff_tab:
            raise Exception(
                self.__class__.__name__
                + f"({mother},{daughter}) combination not in inclusive differential cross sections"
            )
    ...
    def _arange_on_xgrid(self, incl_cs):
        ...
        if len(cs.shape) > 1 and cs.shape[0] != nxbins:
            raise Exception("One dimensional cross section expected, instead got", cs.shape, "\n", cs)

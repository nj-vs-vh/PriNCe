...
class CosmicRaySource(metaclass=ABCMeta):
    def __init__(self, prince_run, ncoids=None, params=None, norm=1.0, m="flat", *args, **kwargs):
    ...
    def _compute_injection_grid(self):
        ...
        for pid in self.ncoids:
            ...
            info(4, f"Injecting particle {pid} with parameters {params}")
            inj_spec = self.spec_man.ncoid2sref[pid]
            self.injection_grid[inj_spec.sl] = self.injection_spectrum(pid, self.cr_grid, params)
    ...
    def evolution(self, z) -> float:
        ...
        """
        from .cosmology import agn_rate, grb_rate_wp, star_formation_rate

        # Check if negative z is called
        if z < 0:
            raise Exception(f"Source evolution not defined for negative z = {z}")
...
class AugerFitSource(CosmicRaySource):
    ...
    def injection_spectrum(self, pid, energy, params):
        ...
        emax = rcut * inj_spec.Z
        e_k = A * energy
        result = relnorm * A * (e_k / 1e9) ** (-spectral_index) * np.where(e_k < emax, 1.0, np.exp(1 - e_k / emax))

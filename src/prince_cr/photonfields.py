...
class CIBFranceschiniZ0(PhotonField):
    ...
    def get_photon_density(self, E, z):
        ...
        if z > 0:
            raise Exception(
                self.__class__.__name__ + "get_photon_density(): " + "Redshift z > 0 not supported by this class"
            )
...
if __name__ == "__main__":
    ...
    ax.loglog(erange, erange * cmb.get_photon_density(erange, z=0.0), ls="-", lw=2, color="k")
    ...
    fig, ax = plt.subplots(1, 1, figsize=(4, 3))
    ax.loglog(erange, erange * inoue.get_photon_density(erange, z=0.0), ls="-", lw=2, color="k")
    ax.set_ylabel(r"$\epsilon$ d$n/$d$\epsilon$ cm$^{-3}$")
    ax.set_xlabel(r"Photon energy $\epsilon$ (GeV)")

    mcomb = CombinedPhotonField([CMBPhotonSpectrum, CIBInoue2D])
    fig, ax = plt.subplots(1, 1, figsize=(4, 3))
    ax.loglog(erange, erange * mcomb.get_photon_density(erange, 0.0), ls="-", lw=2, color="k")
    ax.fill_between(
        erange,

...
from prince_cr.util import get_2Dinterp_object, get_AZN, get_interp_object, info
...
class ResponseFunction:
    ...
    def get_full(self, mother, daughter, ygrid, xgrid=None):
        ...
        if get_AZN(mother)[0] < get_AZN(daughter)[0]:
            info(3, f"WARNING: channel {mother} -> {daughter} with daughter heavier than mother!")
        ...
        if (mother, daughter) in self.incl_intp:
            ...
            res += self.incl_diff_intp[(mother, daughter)].inteval(xgrid, ygrid, grid=False)
    ...
    def get_channel(self, mother, daughter=None):
        ...
        if daughter is not None:
            if (mother, daughter) in self.incl_diff_idcs:
                ...
                raise Exception(f"Unknown inclusive channel {mother} -> {daughter} for this model")
        else:
    ...
    def _precompute_interpolators(self):
        ...
        for mother, daughter in self.incl_idcs:
            self.incl_intp[(mother, daughter)] = get_interp_object(*self.get_channel(mother, daughter))
